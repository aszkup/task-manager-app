...
class UploadFileWorker(
    ...
    private val notificationUtils: NotificationUtils by instance()
    ...
    private fun sendFile(task: Task): Task {
        ...
        fileDescriptor?.let {
            ...
            notificationId = notificationUtils.createNotification(task.name, fileName)
            ...
            notificationId?.let { notificationUtils.markNotificationFinished(it) }
    ...
    private fun getProgressListener(fileSize: Long) =
            IOUtil.ProgressListener { uploadedBytes ->
                ...
                notificationId?.let { notificationUtils.updateProgress(progress.toInt(), it) }
            }
